...
internal sealed class GodotArrayAndDictionaryPtrMarshallerWriter : PtrMarshallerWriter
{
    /// <summary>
    ...
    public override bool NeedsCleanupForParameters => !_marshallableType.IsByRefLike;
    public override bool NeedsCleanupForReturnValue => false;

    public GodotArrayAndDictionaryPtrMarshallerWriter(TypeInfo marshallableType, TypeInfo? unmanagedPointerType = null)
    {
    ...
    {
        if (!_marshallableType.IsByRefLike)
        {
            writer.Write($"{_unmanagedPointerType.PointedAtType!.FullNameWithGlobal} {destination}");
            writer.Write($" = {source} is not null");
            writer.Write($" ? {source}.NativeValue.DangerousSelfRef");
            writer.WriteLine($" : {_unmanagedPointerType.PointedAtType!.FullNameWithGlobal}.Create();");
            return true;
        }
    ...
    {
        // Nothing to free. We only need to free the value if it's used as a parameter.
    }

    protected override void WriteFreeForParameterCore(IndentedTextWriter writer, TypeInfo type, string parameterName, string source)
    {
        if (!_marshallableType.IsByRefLike)
        {
